...
// A process the agent spawned can hold its pipe open after the agent exits; waiting on that pipe would be unbounded.
#[test]
...
// An agent can crash and still exit 0; stderr is then the only account of the fault.
#[test]
